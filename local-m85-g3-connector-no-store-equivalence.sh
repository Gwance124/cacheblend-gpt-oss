#!/usr/bin/env bash
# Deterministic A/B/A for connector absence versus diagnostic no-store presence.
...
readonly CACHEBLEND_POINTER=/mnt/nvme3n1/mlee/cacheblend-gpt-oss/artifacts/solab-g3-m8.5-connector-no-store-equivalence-20260818.current
readonly CACHEBLEND_BASE_DIR=/mnt/nvme3n1/mlee/cacheblend-gpt-oss/artifacts/solab-g3-m8.5-connector-no-store-equivalence-20260818
...
main() {
  ...
  # A/B/A changes only connector presence; its connector arm explicitly
  # suppresses post-forward gather/store/publication.
  ...
  .venv/bin/python scripts/evaluate_connector_presence_equivalence.py \
    ...
    --require-zero-store \
    --output "$CACHEBLEND_RUN_DIR/connector-no-store-verdict.json" \
    | tee "$CACHEBLEND_RUN_DIR/connector-no-store-verdict.txt"
